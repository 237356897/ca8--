...
{
    public class MelsecPLCTcp : MelsecPLC, IDisposable
    {
        private ILog log = LogManager.GetLogger(typeof(MelsecPLCTcp));
        ...
        private AxACTETHERLib.AxActLCPUTCP plc;
        ...
        public MelsecPLCTcp(string ip)
        {
            Open(ip);
            if (ConnectResult != 0)
            {
                MessageBox.Show(string.Format("三菱PLC  IP：{0}通讯异常--Error：{1}", ip, ConnectResult.ToString("X")));
                log.ErrorFormat("三菱PLC  IP：{0}通讯异常--Error：{1}", ip, ConnectResult.ToString("X"));
            }
        }
        ...
        {
            plc = new AxACTETHERLib.AxActLCPUTCP();
            plc.CreateControl();
            plc.ActHostAddress = address;
